        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          <a href="#inicio" onClick={(e) => handleNavClick(e, "#inicio")}
            ...
            <div className="flex flex-col leading-none">
              <span className="font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-violet-400 tracking-[0.05em] text-sm" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>BHADOTT</span>
              <span className="text-[10px] text-slate-500 tracking-[0.15em] uppercase font-medium">Studio</span>
            </div>
          </a>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)}
            ...
            <AnimatePresence mode="wait" initial={false}>
              {mobileOpen ? (
                <motion.div key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.15 }}><X size={20} aria-hidden="true" /></motion.div>
              ) : (
                <motion.div key="menu" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.15 }}><Menu size={20} aria-hidden="true" /></motion.div>
              )}
            </AnimatePresence>
          </button>
        </div>
        ...
        {mobileOpen && (
          <motion.div id="mobile-menu" role="dialog" aria-label="Mobile navigation"
            ...
            <nav className="px-4 py-4 flex flex-col gap-1" aria-label="Mobile menu">
              {navLinks.map((link, i) => (
                <motion.a key={link.label} href={link.href} onClick={(e) => handleNavClick(e, link.href)}
                  initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04, duration: 0.2 }}
              ...
              <motion.a href="#contato" onClick={(e) => handleNavClick(e, "#contato")}
                initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: navLinks.length * 0.04, duration: 0.2 }}
