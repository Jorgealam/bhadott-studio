...
// comentários de clientes. Dados vindos do Cloudflare D1 via
// Worker (API_BASE configurado em VITE_AGRO_API_URL).
// Fallback para seed data quando API não está disponível.
// ============================================================
...
import {
  ...
  Filter, ExternalLink, Quote, CheckCircle2, Sprout, Loader2,
  Wifi, WifiOff,
} from "lucide-react"
...
import {
  fetchPosts, fetchComments, apiSubmitComment,
  isApiConfigured,
} from "../lib/agroApi"
...
// ── Data hook ────────────────────────────────────────────────
function useAgroPublic() {
  const [posts,    setPosts]    = useState(seedPosts)
  const [comments, setComments] = useState(seedComments.filter(c => c.approved))
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const [p, c] = await Promise.all([fetchPosts(), fetchComments()])
      setPosts(p)
      setComments(c)
      setOnline(true)
    } catch {
  ...
  return { posts, comments, loading, online, refresh: load }
}
    ...
    if (v?.provider === "youtube") {
      return (
        <div className="mb-5 aspect-video overflow-hidden rounded-xl border border-white/8">
          <iframe src={v.embed} title={media.label || "Vídeo"} className="h-full w-full"
  ...
  if (media.kind === "document") {
    return (
      <a href={media.url} target="_blank" rel="noopener noreferrer"
        className="mb-5 flex items-center gap-3 rounded-xl border border-white/8 p-4 hover:border-blue-500/30 transition-colors"
...
function PostCard({ post, index, inView, onOpen }) {
  ...
  return (
    <motion.article
      ...
      transition={{ delay: Math.min(index, 8) * 0.06, duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
      whileHover={{ y: -6 }}
      onClick={() => onOpen(post)}
      role="button" tabIndex={0}
      onKeyDown={(e) => e.key === "Enter" && onOpen(post)}
      aria-label={`Abrir: ${post.title}`}
      className="group relative flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-white/8 hover:border-white/18 transition-all duration-300"
    ...
    >
      ...
      <div className="absolute top-0 inset-x-0 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-400 rounded-t-2xl pointer-events-none"
        style={{ background: `linear-gradient(to right, transparent, ${c.hex}80, transparent)` }} aria-hidden="true" />
      ...
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <div className="mb-3 flex items-center gap-2 flex-wrap">
          ...
          <span className="ml-auto text-[11px] text-slate-600 shrink-0">{timeAgo(post.date)}</span>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        exit={{ opacity: 0, y: 24, scale: 0.98 }} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}
        ...
        <button onClick={onClose}
          className="focus-ring absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-lg border border-white/8 text-slate-400 hover:bg-white/5 hover:text-white transition-colors"
          aria-label="Fechar">
...
function Stars({ value = 5, size = 14, onSet }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <button key={n} type={onSet ? "button" : undefined} onClick={onSet ? () => onSet(n) : undefined}
          className={onSet ? "focus-ring rounded" : "pointer-events-none"}
          aria-label={onSet ? `${n} estrela${n > 1 ? "s" : ""}` : undefined} tabIndex={onSet ? 0 : -1}>
          <Star size={size} className={n <= value ? "text-yellow-400" : "text-slate-700"}
            fill={n <= value ? "currentColor" : "none"} aria-hidden="true" />
        </button>
      ))}
    </div>
  )
...
function CommentCard({ comment, index, inView }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
      ...
      className="relative rounded-2xl border border-white/6 p-5 sm:p-6"
...
function CommentForm({ onSubmitted }) {
  ...
  const iStyle = {
    ...
    color: "#e2e8f0", borderRadius: "12px", padding: "12px 16px", fontSize: "14px", width: "100%", outline: "none",
  }

  const submit = async (e) => {
    e.preventDefault()
    if (!form.author.trim() || !form.text.trim()) { setError("Preencha seu nome e o comentário."); return }
    setError(""); setBusy(true)
    try {
      await apiSubmitComment({ id: makeId("c"), author: form.author.trim(), role: form.role.trim(),
        rating: form.rating, text: form.text.trim(), date: new Date().toISOString() })
      setSent(true)
      onSubmitted?.()
    } catch (err) {
    ...
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-green-500/15 px-6 py-12 text-center" style={{ background: "rgba(34,197,94,0.04)" }}>
        ...
        <p className="max-w-xs text-sm text-slate-400">
          Obrigado! Seu comentário será visível após aprovação.
        </p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-4 rounded-2xl border border-white/6 p-6" style={{ background: "rgba(255,255,255,0.02)" }}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <input value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} placeholder="Seu nome" style={iStyle} />
        <input value={form.role}   onChange={(e) => setForm({ ...form, role:   e.target.value })} placeholder="Cidade / atividade (opcional)" style={iStyle} />
      </div>
      ...
      <textarea value={form.text} onChange={(e) => setForm({ ...form, text: e.target.value })}
        placeholder="Conte sua experiência com o BHADOTT Agro Solutions..." rows={4} style={{ ...iStyle, resize: "vertical" }} />
      {error && <p className="text-xs text-red-400">{error}</p>}
      <motion.button type="submit" whileHover={{ scale: busy ? 1 : 1.02 }} whileTap={{ scale: 0.98 }}
        disabled={busy}
...
// ── Hero Stats ───────────────────────────────────────────────
function HeroStats({ posts, comments }) {
  const s = [
    { label: "Publicações", value: posts.length },
    { label: "Anúncios",    value: posts.filter(p => p.type === "anuncio").length },
    { label: "Avaliações",  value: comments.length },
  ]
  return (
    <div className="mx-auto mt-10 grid max-w-md grid-cols-3 gap-4">
      {s.map((x) => (
        <div key={x.label} className="text-center">
          <div className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-violet-400"
            style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{x.value}</div>
...
export default function AgroSolutions() {
  const { posts, comments, loading, online, refresh } = useAgroPublic()
  ...
  const commInView  = useInView(commRef, { once: true, margin: "-40px" })
  ...
  return (
    <PageLayout backLabel="Voltar para Home" backTo="/">
      ...
      >
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            ...
            className="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-300 hover:text-white transition-colors"
            style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <MessageSquare size={15} aria-hidden="true" /> Ver comentários
          </motion.button>
          <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
            onClick={() => navigate("/studio")}
            className="inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-400 hover:text-blue-400 transition-colors"
            style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}
            title="Painel de gestão">
            <Lock size={14} aria-hidden="true" /> Painel Studio
          </motion.button>
        </div>
        ...
        {isApiConfigured() && !loading && (
          <div className="mt-4 flex justify-center">
            <span className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold ${
              ...
              {online
                ? <><Wifi size={11} aria-hidden="true" /> Ao vivo — Cloudflare D1</>
                : <><WifiOff size={11} aria-hidden="true" /> Exibindo dados em cache</>
              }
            </span>
          </div>
        )}

        <HeroStats posts={posts} comments={comments} />
      </PageHero>
        ...
        <div ref={feedRef} className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            <div className="flex flex-col items-center py-20 gap-4">
              <Loader2 size={32} className="text-blue-400 animate-spin" aria-hidden="true" />
              <span className="text-sm text-slate-500">Carregando publicações...</span>
            </div>
          ) : filtered.length > 0 ? (
      ...
      {/* ── COMENTÁRIOS ── */}
      <section id="comentarios" ref={commRef} className="relative py-16 sm:py-20">
        <div className="absolute inset-0" style={{ background: "#020617" }} aria-hidden="true" />
        <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">

          <motion.div initial={{ opacity: 0, y: 16 }} animate={commInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
          ...
          {comments.length > 0 && (
            <div className="mb-12 grid grid-cols-1 gap-5 md:grid-cols-3">
              {comments.map((c, i) => (
                <CommentCard key={c.id} comment={c} index={i} inView={commInView} />
              ))}
            </div>
          )}

          <div className="mx-auto max-w-xl">
            <CommentForm onSubmitted={refresh} />
          </div>
        </div>
      </section>
