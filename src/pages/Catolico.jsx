...
function CatholicAppInvite() {
  ...
  const enableNotifications = async () => {
    if (typeof Notification === "undefined") { setNotificationNotice("Este navegador não oferece notificações para este aplicativo."); return }
    const permission = await Notification.requestPermission()
    setNotificationState(permission)
    setNotificationNotice(permission === "granted" ? "Notificações permitidas. Agora faça o teste." : permission === "denied" ? "A notificação foi bloqueada. Ative-a nas configurações do navegador do celular." : "A permissão ainda não foi concedida.")
  }
  const testNotification = async () => {
    if (notificationState !== "granted") { await enableNotifications(); return }
    if (!("serviceWorker" in navigator)) { setNotificationNotice("Este navegador não permite o teste pelo aplicativo."); return }
    const registration = await navigator.serviceWorker.ready
    await registration.showNotification("BHADOTT Católico", { body: "Teste concluído: o lembrete do Evangelho está autorizado neste celular.", icon: `${import.meta.env.BASE_URL}favicon.svg`, badge: `${import.meta.env.BASE_URL}favicon.svg`, tag: "bhadott-notification-test" })
    setNotificationNotice("Notificação de teste enviada. Confira a barra de notificações do celular.")
  }
  const dismiss = () => { window.localStorage.setItem("bhadott-pwa-invite-hidden", "yes"); setHidden(true) }
  return <div className="border-b border-[#ded3e4] bg-[#f3eaf8]"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4"><div className="flex flex-col lg:flex-row lg:items-center gap-4"><div className="flex-1"><span className="text-[10px] uppercase tracking-wider font-bold text-[#5b159d]">Aplicativo BHADOTT</span><h3 className="font-bold text-[#2c2030] mt-1">Leve a Área Católica com você</h3><p className="text-xs text-[#756b78] mt-1">O lembrete atual aparece quando você abre o aplicativo em um novo dia. Use o teste abaixo para conferir a permissão do celular.</p><p className="text-[11px] font-semibold text-[#806d87] mt-2">Estado: {notificationState === "granted" ? "permitida" : notificationState === "denied" ? "bloqueada" : notificationState === "unsupported" ? "não disponível" : "aguardando autorização"}</p></div><div className="flex flex-wrap gap-2">{installPrompt && <button onClick={install} className="inline-flex items-center gap-2 rounded-xl bg-[#5b159d] text-white px-4 py-3 text-xs font-bold"><Download size={14} /> Instalar aplicativo</button>}{notificationState !== "granted" && notificationState !== "unsupported" && <button onClick={enableNotifications} className="inline-flex items-center gap-2 rounded-xl bg-[#8a6924] text-white px-4 py-3 text-xs font-bold"><Bell size={14} /> Permitir notificação</button>}{notificationState === "granted" && <button onClick={testNotification} className="inline-flex items-center gap-2 rounded-xl bg-[#8a6924] text-white px-4 py-3 text-xs font-bold"><Bell size={14} /> Testar notificação</button>}<button onClick={dismiss} className="inline-flex items-center gap-2 rounded-xl border border-[#d8cae0] bg-white px-4 py-3 text-xs font-bold text-[#756b78]"><X size={14} /> Ocultar</button></div></div>{notificationNotice && <p className="mt-3 rounded-xl bg-white px-4 py-3 text-xs font-semibold text-[#5b159d]">{notificationNotice}</p>}</div></div>
}
...
function DailyLiturgyView() {
  ...
  const portugueseReadingUrl = "https://liturgiadiaria.edicoescnbb.com.br/"
  ...
  return <div className="rounded-[2rem] bg-[#fbf8f2] text-[#271e2b] p-5 sm:p-9 lg:p-12">
    ...
    {!loading && liturgy && <><div className="max-w-3xl mx-auto mt-9 rounded-2xl border border-[#d4c28f] bg-[#fff4d8] p-5 text-center"><h3 className="font-bold text-[#553b08]">Para ler no celular</h3><p className="text-sm text-[#735b2a] mt-2">Abaixo aparecem as referências corretas do dia. Toque em <strong>Ler o texto em português</strong> para abrir a leitura católica completa.</p><a href={portugueseReadingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 rounded-xl bg-[#8a6924] text-white px-5 py-3 text-sm font-bold">Ler o texto em português <ExternalLink size={14} /></a></div><div className="grid lg:grid-cols-[290px_1fr] gap-5 mt-6 items-start"><aside className="rounded-3xl border border-[#e2d8e8] bg-white p-5 lg:sticky lg:top-40"><span className="text-[10px] uppercase tracking-wider text-[#8a6924]">Celebração</span><h3 className="text-2xl font-bold mt-2" style={{ fontFamily: "Georgia, serif" }}>{celebration}</h3><p className="text-sm text-[#766b79] mt-2">Cor litúrgica: <strong>{liturgicalColors[liturgy.colore] || liturgy.colore}</strong></p><div className="space-y-2 mt-6">{readings.map(([type, reading]) => <a key={type} href={`#reading-${type}`} className="block rounded-xl bg-[#f4edf8] p-3"><strong className="block text-xs text-[#795911]">{readingLabels[type] || type}</strong><span className="text-sm">{reading.riferimento}</span></a>)}</div></aside><main className="space-y-4">{readings.map(([type, reading]) => <article id={`reading-${type}`} key={type} className={`rounded-3xl border p-6 sm:p-8 ${type === "vangelo" ? "border-[#caa64e] bg-[#fffaf0]" : "border-[#e2d8e8] bg-white"}`}><span className="text-xs uppercase tracking-wider font-bold text-[#8a6924]">{readingLabels[type] || type}</span><h3 className="text-3xl sm:text-4xl font-bold mt-3" style={{ fontFamily: "Georgia, serif" }}>{reading.riferimento}</h3><a href={portugueseReadingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-5 rounded-xl border border-[#d4c28f] bg-white px-4 py-3 text-sm font-bold text-[#795911]">Ler o texto em português <ExternalLink size={14} /></a>{type === "vangelo" && <div className="mt-6 rounded-2xl bg-[#f1e8f7] p-5"><strong className="text-sm">Para meditar</strong><p className="text-sm text-[#5f5365] leading-relaxed mt-2">O que Jesus me convida a escutar, transformar ou praticar neste dia?</p></div>}</article>)}</main></div></>}
    <div className="max-w-3xl mx-auto mt-8 rounded-3xl border border-[#e2d8e8] bg-white p-6"><h3 className="font-bold">Leitura católica em português</h3><p className="text-sm text-[#766b79] mt-2">As referências são atualizadas automaticamente. Enquanto preparamos uma tradução católica de domínio público para leitura dentro do portal, o texto integral abre na Liturgia Diária da CNBB.</p><div className="flex flex-wrap gap-3 mt-5"><a href={portugueseReadingUrl} target="_blank" rel="noopener noreferrer" className="rounded-xl bg-[#8a6924] text-white px-4 py-3 text-sm font-bold">Ler na CNBB <ExternalLink size={14} className="inline ml-1" /></a><button onClick={copyReading} className="rounded-xl border border-[#d4c28f] px-4 py-3 text-sm font-bold"><Copy size={14} className="inline mr-1" /> Copiar referência</button><button onClick={shareReading} className="rounded-xl bg-[#5b159d] text-white px-4 py-3 text-sm font-bold"><Share2 size={14} className="inline mr-1" /> Compartilhar</button></div>{notice && <p className="text-sm text-emerald-700 mt-3">{notice}</p>}</div>
...
function GuidedRosary({ title, devotion }) {
  const [position, setPosition] = useState(0)
  const [bead, setBead] = useState(0)
  const isRosary = devotion === "rosario"
  const stagesPerPart = isRosary ? 5 : 3
  const parts = isRosary ? 5 : 9
  const total = stagesPerPart * parts
  const partIndex = Math.min(Math.floor(position / stagesPerPart), parts - 1)
  const stage = position % stagesPerPart
  const mystery = isRosary ? rosaryScripture[title][partIndex] : null
  const choir = isRosary ? null : angelicChoirs[partIndex]
  const prayerName = stage === 1 ? "Pai-Nosso" : stage === 2 ? "Ave-Maria" : isRosary && stage === 3 ? "Glória ao Pai" : null
  const prayer = prayerName ? traditionalPrayers.find((item) => item.title === prayerName) : null
  const maxBeads = isRosary ? 10 : 3
  const isBeadStage = stage === 2
  useEffect(() => { setPosition(0); setBead(0) }, [title, devotion])
  const move = (amount) => { setPosition((value) => Math.max(0, Math.min(total - 1, value + amount))); setBead(0) }
  const stageTitle = stage === 0 ? (isRosary ? "Leitura para contemplar" : `Saudação aos ${choir[0]}`) : prayerName || "Oração do mistério"
  return <section className="rounded-[2rem] border border-[#d8c9df] bg-gradient-to-br from-[#fffdf8] to-[#f4eaf8] p-5 sm:p-9 mb-7"><div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><span className="text-[10px] uppercase tracking-wider font-bold text-[#8a6924]">Modo guiado · {partIndex + 1}{isRosary ? "º mistério" : "ª saudação"}</span><h3 className="text-xl sm:text-2xl font-bold text-[#2c2030] mt-1">{isRosary ? mystery[0] : `Em honra ao coro dos ${choir[0]}`}</h3></div><strong className="text-sm text-[#5b159d]">{position + 1} de {total}</strong></div><div className="h-1.5 rounded-full bg-white mt-5 overflow-hidden"><div className="h-full bg-gradient-to-r from-[#8a6924] to-[#5b159d] transition-all" style={{ width: `${((position + 1) / total) * 100}%` }} /></div><article className="min-h-[390px] rounded-3xl border border-[#e1d6e6] bg-white p-6 sm:p-10 mt-5 flex flex-col justify-center text-center"><span className="text-xs uppercase tracking-[0.18em] font-bold text-[#8a6924]">{stageTitle}</span>{stage === 0 ? isRosary ? <><h4 className="text-3xl font-bold text-[#2c2030] mt-4" style={{ fontFamily: "Georgia, serif" }}>{mystery[1]}</h4><p className="max-w-2xl mx-auto text-base sm:text-lg text-[#554a59] leading-8 mt-6" style={{ fontFamily: "Georgia, serif" }}>{rosaryNarratives[mystery[0]]}</p><p className="text-sm text-[#806f86] mt-6"><strong>Para guardar no coração:</strong> {mystery[2]}</p><p className="text-[11px] text-[#95889a] mt-4">Meditação narrativa católica baseada na passagem indicada.</p></> : <><h4 className="text-3xl font-bold text-[#2c2030] mt-4">{choir[0]}</h4><p className="max-w-2xl mx-auto text-lg text-[#554a59] leading-8 mt-7">{choir[1]}</p></> : stage === 4 ? <><h4 className="text-3xl font-bold text-[#2c2030] mt-4">Entregue este mistério</h4><p className="max-w-2xl mx-auto text-lg text-[#554a59] leading-8 mt-7">Senhor Jesus, fazei frutificar em nossa vida aquilo que contemplamos. Conduzi-nos na fé, na esperança e na caridade. Amém.</p></> : <><h4 className="text-3xl font-bold text-[#2c2030] mt-4" style={{ fontFamily: "Georgia, serif" }}>{stageTitle}</h4><p className="max-w-2xl mx-auto text-lg text-[#554a59] leading-8 mt-7" style={{ fontFamily: "Georgia, serif" }}>{prayer?.text}</p>{isBeadStage && <div className="mt-7"><div className="flex flex-wrap justify-center gap-2">{Array.from({ length: maxBeads }, (_, index) => <span key={index} className={`w-9 h-9 rounded-full border grid place-items-center text-xs font-bold ${index < bead ? "bg-[#5b159d] text-white border-[#5b159d]" : "bg-[#f8f2fa] text-[#806d87] border-[#d8c9df]"}`}>{index + 1}</span>)}</div><button onClick={() => setBead((value) => Math.min(maxBeads, value + 1))} className="mt-5 rounded-xl bg-[#8a6924] text-white px-5 py-3 text-sm font-bold">Rezei esta Ave-Maria · {bead}/{maxBeads}</button></div>}</>}</article><div className="flex items-center justify-between gap-3 mt-5"><button onClick={() => move(-1)} disabled={position === 0} className="rounded-xl border border-[#d9cce0] bg-white px-4 py-3 text-sm font-bold disabled:opacity-30"><ChevronLeft size={15} className="inline" /> Voltar</button><button onClick={() => move(1)} disabled={position === total - 1 || (isBeadStage && bead < maxBeads)} className="rounded-xl bg-[#5b159d] text-white px-5 py-3 text-sm font-bold disabled:opacity-30">Continuar <ChevronRight size={15} className="inline" /></button></div></section>
}
...
function RosaryView() {
  ...
  const reset = (nextDevotion) => setDevotion(nextDevotion)
  return <>
    ...
    {devotion === "rosario" && <div className="flex gap-2 overflow-x-auto pb-2 mb-6">{Object.keys(rosaryScripture).map((title) => <button key={title} onClick={() => setSetTitle(title)} className={`flex-shrink-0 rounded-xl px-4 py-3 text-xs font-bold border ${setTitle === title ? "bg-[#f1e4f7] text-[#5b159d] border-[#b997c9]" : "bg-white text-[#756b78] border-[#e1d6e6]"}`}>{title}</button>)}</div>}
    <GuidedRosary title={setTitle} devotion={devotion} />
